...
void SHA256::encode() {
  ...
  for (int i = 16; i < rounds; i++) {
    ...
    cnf.varName(s1[i], "s1_" + to_string(i) + "_z0");
    ...
    cnf.varName(wcarry[i], "add_w" + to_string(i) + "_x2", -1);
    cnf.varName(wCarry[i], "add_w" + to_string(i) + "_x3", -2);
  ...
  for (int i = 0; i < rounds; i++) {
    ...
    Sigma0(sigma0[i], A[i + 3], "sigma0_" + std::to_string(i) + "_");
    ...
    cnf.varName(r0carry[i], "add_T" + to_string(i) + "_x2", -1);
    cnf.varName(r0Carry[i], "add_T" + to_string(i) + "_x3", -2);
    ...
    cnf.varName(r2Carry[i], "add_A" + to_string(i + 4) + "_x3", -2);
...
void SHA256::Sigma0(int *z, int *x, std::string prefix) {
...
void SHA256::Sigma1(int *z, int *x, std::string prefix) {
  int r1[32], r2[32], r3[32];
  cnf.rotr(r1, x, 6);
  cnf.rotr(r2, x, 11);
  cnf.rotr(r3, x, 25);
